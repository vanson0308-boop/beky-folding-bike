...
(function () {
  ...
  // 生活照輪播：交叉淡入淡出，requestAnimationFrame 計時，滑鼠移入暫停
  ...
  if (gallery) {
    ...
    let index = 0, last = performance.now(), paused = false;
    ...
    gallery.addEventListener('mouseleave', () => { paused = false; last = performance.now(); });
    const tick = now => {
      if (!paused && now - last > GALLERY_INTERVAL) show(index + 1);
